...
res_60x72()
{
	...
	for (row = 0; row < rows; row += LOW_RES_ROWS)
	{
		...
		fprintf (stdout, "\033K%c%c", cols % 256, cols / 256);
...
res_120x144()
{
	...
	for (row = 0; row < rows; row += HIGH_RES_ROWS)
	{
		...
		for (i = 0; i < npins; ++i)
			pbm_readpbmrow(ifp, bP[i] = bitrows[i], cols, format);
		fprintf (stdout, "\033L%c%c", cols % 256, cols / 256);
		for (col = 0; col < cols; ++col)
		{
			item = 0;
			/* even rows */
		...
		putchar('\n');			/* flush buffer */
		fprintf (stdout, "\033J\001");		/* 1/144 down */
		fprintf (stdout, "\033L%c%c", cols % 256, cols / 256);
		for (col = 0; col < cols; ++col)
		{
			item = 0;
		...
		putchar('\n');			/* flush buffer */
		fprintf (stdout, "\033J\017");		/* 15/144 down */
	}
}
