...
main( argc, argv )
    ...
    fprintf (stdout,  "#define %s_format %d\n", out_name, 1 );
    fprintf (stdout,  "#define %s_width  %d\n", out_name, cols );
    fprintf (stdout,  "#define %s_height %d\n", out_name, rows );
    fprintf (stdout,  "#define %s_ncolors %d\n", out_name, ncolors );
    fprintf (stdout,  "#define %s_chars_per_pixel %d\n", out_name, charspp );

    /* Write out the ascii colormap. */
    fprintf (stdout,  "static char *%s_colors[] = {\n", out_name );
    for ( i = 0; i < ncolors; ++i )
	{
	fprintf (stdout,  "   \"%s\", \"%s\"", cmap[i].cixel, cmap[i].rgbname );
	if ( i != ncolors - 1 )
	    fprintf (stdout,  ",\n" );
	else
	    fprintf (stdout,  "\n" );
    ...
    fprintf (stdout,  "static char *%s_pixels[] = {\n", out_name );
    for ( row = 0; row < rows; ++row )
	{
	fprintf (stdout,  "\"" );
	for ( col = 0, pP = pixels[row]; col < cols; ++col, ++pP )
	    fprintf (stdout,  "%s", cmap[ppm_lookupcolor(cht, pP)].cixel );
	if ( row != rows - 1 )
	    fprintf (stdout,  "\",\n" );
	else
	    fprintf (stdout,  "\"\n" ); 
