...
main( argc, argv )
    ...
    while ( argn < argc && argv[argn][0] == '-' && argv[argn][1] != '\0' )
        ...
        if ( pm_keymatch( argv[argn], "-interlace", 2 ) )
            ...
            interlace = 0;
				...
				pm_error( "cannot use a null colormap" );
	    		}
        else
            pm_usage( usage );
        ++argn;
    ...
    if ( mappixels )
		...
		chv = ppm_computecolorhist(
	    	mappixels, mapcols, maprows, MAXCOLORS, &colors );
   ...
   for ( i = 0; i < colors; ++i )
		...
		if ( mapmaxval == 255 )
	    	...
	    	{
	    	Red[i] = (int) PPM_GETR( chv[i].color ) * 255 / mapmaxval;
	    	Green[i] = (int) PPM_GETG( chv[i].color ) * 255 / mapmaxval;
	    	Blue[i] = (int) PPM_GETB( chv[i].color ) * 255 / mapmaxval;
	    	}
		}
