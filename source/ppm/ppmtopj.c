...
char *argv[];
{
	...
	(void) fprintf (stdout, C_RESET);
	/*
	 * Set the resolution before begin raster otherwise it
	 * does not work.
	 */
	(void) fprintf (stdout, C_RESOLUTION, resolution);
	(void) fprintf (stdout, C_BEGIN_RASTER, posscale);
	if (xoff)
	    (void) fprintf (stdout, C_MOVE_X, xoff);
	if (yoff)
	    (void) fprintf (stdout, C_MOVE_Y, yoff);
	(void) fprintf (stdout, C_TRANS_MODE, mode);
	(void) fprintf (stdout, C_RENDER, render_mode);
	(void) fprintf (stdout, C_BACK_SCALE, back_scale);
	(void) fprintf (stdout, C_GAMMA,	 gamma);
	(void) fprintf (stdout, C_IMAGE_WIDTH, cols);
	(void) fprintf (stdout, C_IMAGE_HEIGHT, rows);
	(void) fprintf (stdout, C_DATA_PLANES, 3);
		...
		(void) fprintf (stdout, p == 2 ? C_LAST_PLANE : C_SEND_PLANE, k);
		(void) fwrite(op, 1, k, stdout);
	    }
	(void) fprintf (stdout, C_END_RASTER, C_END_RASTER_UNUSED);
	pm_close (stdout);
	exit(0);
}
