...
main( argc, argv )
    ...
    fprintf (stdout,  "names = case_sensitive\n" );
    fprintf (stdout,  "include file 'XmAppl.uil';\n" );
    ...
    fprintf (stdout,  "\n" );
    fprintf (stdout,  "value\n" );
    fprintf (stdout,  "  %s_rgb : color_table (\n", out_name );
    printed_one = 0;
    for ( i = 0; i < ncolors; ++i )
	if ( cmap[i].barname != (char*) 0 )
	    {
	    if ( printed_one )
		fprintf (stdout,  ",\n" );
	    fprintf (stdout,  "    %s = '%s'", cmap[i].barname, cmap[i].cixel );
	    printed_one = 1;
	    }     
    fprintf (stdout,  "\n    );\n" );
