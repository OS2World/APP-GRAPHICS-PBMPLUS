...
WriteHeader()
    {
    if ( margin == 1 )
	fprintf (stdout,  "%c%d;%ds", CSI, 14, 72 );
    fprintf (stdout,  "%c", DCS );  /* start with Device Control String */
    fprintf (stdout,  "0;0;8q" );   /* Horizontal Grid Size at 1/90" and graphics On */
...
WriteColorMap( chv, colors, maxval )
	...
	fprintf (stdout,  "#%d;2;%d;%d;%d", colornum,
...
WriteRawImage( cht, rows, cols )
	...
	for ( colnum = 0, pP = pixels[rownum]; colnum < cols; ++colnum, ++pP )
	    fprintf (stdout,  "#%d%c", ppm_lookupcolor(cht, pP), sixel[b] );
	fprintf (stdout,  "$\n" );   /* Carriage Return */
	if ( b == 5 )
...
WritePackedImage( cht, rows, cols )
	...
	for ( colnum = 0, pP = pixels[rownum]; colnum < cols; ++colnum, ++pP )
		...
		if ( repeat == 1 )
		    fprintf (stdout,  "#%d%c", thiscolor, sixel[b] );
		else
		    fprintf (stdout,  "#%d!%d%c", thiscolor, repeat, sixel[b] );
...
WriteEnd()
    {
    if ( margin == 1 )
	fprintf (stdout,  "%c%d;%ds", CSI, 1, 80 );
    fprintf (stdout,  "%c\n", ST );
    }
