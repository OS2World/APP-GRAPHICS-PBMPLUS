...
#define FS_SCALE 1024
	...
	else if ( pm_keymatch( argv[argn], "-createmap", 2 ) )
		output_map = 1;
	else
	    pm_usage( usage );
	++argn;
...
/* output map */
	if (output_map) {
	/* maprows could be a command line option */
		maprows = 1;
		mapcols = newcolors / maprows;
		if (newcolors % maprows) {
			mapcols ++;
		}
		mappixels = ppm_allocarray (mapcols, maprows);
		for (row = 0; row < maprows; row ++) {
			for (col = 0, pP = mappixels [row]; col < mapcols; col ++, pP ++) {
				if (row * mapcols + col > newcolors) {
					break;
				}
		    	sr = PPM_GETR (colormap [row * mapcols + col].color);
		    	sg = PPM_GETG (colormap [row * mapcols + col].color);
		    	sb = PPM_GETB (colormap [row * mapcols + col].color);
				PPM_ASSIGN (* pP, sr, sg, sb);
			}
		}
		ppm_writeppm (stdout, mappixels, mapcols, maprows, (pixval) 255, 0);
