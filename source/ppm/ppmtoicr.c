...
char* argv[];
{
	...
	(void)fprintf (stdout, "\033^W;%d;%d;%d;%d;%d;%s^",0,0,cols*expand,rows*expand,display,windowname);
	fflush(stdout);
	...
	(void)fprintf (stdout, "\033^M;%d;%d;%d;%s^",0,MAXCOLORS,CLUTCOLORS,windowname);
	thischar = rgb;
	...
	if (rleflag) {	
		...
		(void)fprintf (stdout, "\033^R;%d;%d;%d;%d;%s^",0,i*expand,expand,newxsize,windowname);
		thischar = space;
	...
	/* Otherwise, send out uncompressed pixel data via the slow method */
		...
		for (i = 0; i < rows; i++) {
			(void)fprintf (stdout, "\033^P;%d;%d;%d;%d;%s^",0,i*expand,expand,cols,windowname);
			for (j = 0; j < cols; j++) {
				c  = GetPixel(j,i);
				if (c > 31 && c < 123) {
						putchar(c);
